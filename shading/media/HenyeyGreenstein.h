...
class HenyeyGreenstein : public PhaseFunction {
	public:
		...
		Real Sample_p(const Vec3 &_wo, Vec3 *_wi, const Vec2 &_u) const override {
			Real cosTheta;
			if (std::abs(g) < 1e-3) cosTheta = 1 - 2 * _u.x;
			else {
				const Real sqrTerm = (1 - g * g) / (1 - g + 2 * g * _u.x);
				cosTheta = (1 + g * g - sqrTerm * sqrTerm) / (2 * g);
			}
			const Real sinTheta = std::sqrt(std::max((Real)0, 1 - cosTheta * cosTheta));
			const Real phi = 2 * PI * _u.y;
