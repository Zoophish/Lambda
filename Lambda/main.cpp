...
int main() {
	...
	OrenNayarBRDF mat(&white, 1.4);
	const Real alpha = MicrofacetDistribution::RoughnessToAlpha(0.09);
	BeckmannDistribution d(alpha, alpha);
	FresnelDielectric fres(2.5);
	MicrofacetBRDF tbr(&blue, &d, &fres);
	tbr.etaT = .00001;
	FresnelBSDF mat2(&yellow, 2.4);

	AssetImporter ai;

	ai.Import(&resources, "../content/dragon.obj");
	TriangleMesh mesh2;
	MeshImport::LoadMeshBuffers(ai.scene->mMeshes[1], &mesh2);
	mesh2.bxdf = &mat;
	...
	OrenNayarBRDF mat3(&grid, 2.2);
	mesh3.bxdf = &mat3;
	scene.AddObject(mesh3);

	//ai.Import(&resources, "../content/TopLight.obj");
